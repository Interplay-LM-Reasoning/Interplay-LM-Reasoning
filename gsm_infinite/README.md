...
- 人工插入复杂的behavior
- 插入一些复杂的reasoninig pattern
  - 否定的范式？ counterfactual?
  - 先假设是错的
  - 反向推理？
  - inductive, deductive, abductive reasoning
  - analogy, counterfactual, validation, exploration

- 我觉得需要这样的任务？
 - 目标是看各种模式的泛化？
 - 一开始没有这个pattern后面能不能学得会，这个pattern能否迁移？
 - 是否需要这个pattern？ 非这个pattern不可以的，或者有这个pattern性能更高；不需要这个解法，但是人为能插入也可以。
 - 如果可以构造非XXX就不可以的，那就更好track这个任务
 - 多种解法也可以，预训练更大规模凸显的是其中一种范式，比如inductive reasoning，一些conterfactual的问题，理论上来说inductive reasoning,但是conterfactual 更好

 - 要强调scalable，必须有一套这样的generation，生成无限token的数据

 - model size 只用 BERT级别的model，scale up training data,遵循 scaling law就可以
 - 首先要保证模型确实能学得会，
 - 越practical
 - 想data generator要想泛化性，怎么在不同task；不同难度，不同reasoning pattern上去做
  - 非某某某不可的task
  - 多种解法的task，但是插入不同的reasoning pattern
  - GPT生成直接写死，scalable的东西；
